 ...
 * account_session.created_at < 10min, so it must happen during provider configuration.
 ...
 * Called before enrollTrustedDevice() during provider configuration so a stale token from the
 ...
 * this must be called immediately after a fresh provider configuration. Calling it later
...
export async function enrollTrustedDevice(): Promise<void> {
  try {
    ...
    // of getTrustedDeviceToken() don't need this; only provider configuration does.
    ...
    // Always re-enroll on provider configuration — the existing token may belong to a
    // different account (account-switch without provider configuration). Skipping enrollment
