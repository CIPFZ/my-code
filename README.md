...
- 移除了 `meetsAvailabilityRequirement()` 中的订阅限制检查
- 本地 API Key 模式下所有命令均可使用
- 包括 `/fast`、`/desktop`、`/chrome`、`/voice`、`/usage`、`/upgrade` 等
...
配置文件路径（按优先级）：
1. `CLAUDE_CODE_MODEL_CONFIG` 环境变量
