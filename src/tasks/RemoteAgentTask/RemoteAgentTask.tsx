...
export function formatPreconditionError(error: BackgroundRemoteSessionPrecondition): string {
  switch (error.type) {
    case 'not_logged_in':
      return 'Please update the configured models.config.json and sign in with your Claude.ai account (not Console).';
...
async function restoreRemoteAgentTasksImpl(context: TaskContext): Promise<void> {
  ...
  for (const meta of persisted) {
    ...
    try {
      ...
      // missing OAuth token) are recoverable via provider configuration — the remote
